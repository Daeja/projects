...
package eth.fingerfixer.service;
...
import eth.fingerfixer.bluetooth.ConnectionInfo;
import eth.fingerfixer.bluetooth.TransactionBuilder;
import eth.fingerfixer.bluetooth.TransactionReceiver;
...
public class BTCTemplateService extends Service {
	...
	public ServiceHandler mServiceHandler = new ServiceHandler();
	...
	private void initialize() {
		...
		if (!mBluetoothAdapter.isEnabled()) {
			...
			//mActivityHandler.obtainMessage(Constants.REQUEST_ENABLE_BT,"aa");
		} else {
			if(mBtManager == null) {
				setupBT();
			}
    ...
    public int getState(){ return mBtManager.getState(); }
		...
		public void handleMessage(Message msg) {
			...
			switch(msg.what) {
				...
				case BluetoothManager.MESSAGE_WRITE:
					mActivityHandler.sendEmptyMessage(Constants.MESSAGE_WRITE_CHAT_DATA);
					break;
				...
				if(readCount > 0) {
					String strMsg = new String(readBuf, 0, readCount);
					...
					strMsg = strMsg.concat(s);

					if(strMsg.length()==10){
						//Toast.makeText(getApplicationContext(), strMsg, Toast.LENGTH_SHORT).show();
						//Toast.makeText(getApplicationContext(), strMsg.substring(0,12), Toast.LENGTH_SHORT).show();
						try{
					...
					}else if(strMsg.equals("NO")){
                        mActivityHandler.obtainMessage(Constants.MESSAGE_READ_CHAT_DATA, strMsg).sendToTarget();
                        strMsg = "";
                    }else if(strMsg.equals("YES")){
                        mActivityHandler.obtainMessage(Constants.MESSAGE_READ_CHAT_DATA, strMsg).sendToTarget();
                        strMsg = "";
                    }
					break;
